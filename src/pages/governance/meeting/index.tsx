...
const MeetingCalendar: React.FC = () => {
  ...
  const getMeetingStatusClass = (meeting: Meeting) => {
    ...
    const endTime = meeting.end_time ? dayjs(meeting.end_time) : startTime.add(2, 'hour');
    ...
    <div className={styles.container}>
      <div className={styles.calendarWrapper}>
        ...
        <Drawer
          title={
            <div className={styles.drawerTitle}>
              ...
              {selectedDate.format('YYYY年MM月DD日')} 的会议 ({selectedMeetings.length}场)
            </div>
          }
          width={600}
          open={drawerVisible}
              ...
              {selectedMeetings.map((meeting) => (
                ...
                >
                  <Card.Meta
                    ...
                    description={
                      <div>
                        <Paragraph ellipsis={{ rows: 2 }} className={styles.meetingDescription}>
                          {meeting.description}
                        </Paragraph>
                        <div className={styles.meetingInfo}>
                          <div className={styles.meetingInfoItem}>
                            <Clock size={14} className={styles.meetingInfoIcon} />
                            {dayjs(meeting.start_time).format('HH:mm')} - {meeting.end_time ? dayjs(meeting.end_time).format('HH:mm') : '待定'}
                          </div>
                          <div className={styles.meetingInfoItem}>
                            {meeting.location === '线上会议' ? (
                              <>
                                <Globe size={14} className={styles.meetingInfoIcon} />
                                线上会议
                              </>
                            ) : (
                              <>
                                <MapPin size={14} className={styles.meetingInfoIcon} />
                                {meeting.location || '待定'}
                              </>
                            )}
                          </div>
                          {meeting.participants && (
                            <div className={styles.meetingInfoItem}>
                              <Users size={14} className={styles.meetingInfoIcon} />
        ...
        >
          {selectedMeeting && (
            <div>
              <Title level={4} className={styles.modalMeetingTitle}>{selectedMeeting.title}</Title>
              <Paragraph className={styles.modalDescription}>{selectedMeeting.description}</Paragraph>
              
              <Space direction="vertical" size="middle" className={styles.modalInfo}>
                <div className={styles.modalInfoItem}>
                  <Text className={styles.modalInfoLabel}>开始时间：</Text>
                  <Text className={styles.modalInfoValue}>{dayjs(selectedMeeting.start_time).format('YYYY-MM-DD HH:mm')}</Text>
                </div>
                
                {selectedMeeting.end_time && (
                  <div className={styles.modalInfoItem}>
                    <Text className={styles.modalInfoLabel}>结束时间：</Text>
                    <Text className={styles.modalInfoValue}>{dayjs(selectedMeeting.end_time).format('YYYY-MM-DD HH:mm')}</Text>
                  </div>
                )}
                ...
                {selectedMeeting.participants && (
                  <div className={styles.modalInfoItem}>
                    ...
                    <Text className={styles.modalInfoValue}>{selectedMeeting.participants} 人</Text>
                  </div>
                )}
                
                <div className={styles.modalInfoItem}>
                  <Text className={styles.modalInfoLabel}>会议类型：</Text>
                  <Tag color={getMeetingTypeColor(selectedMeeting.meeting_type)}>
                ...
                {selectedMeeting.organizer && (
                  <div className={styles.modalInfoItem}>
                    ...
                    <Text className={styles.modalInfoValue}>{selectedMeeting.organizer}</Text>
                  </div>
                )}
